...
switch currentSubmission
    ...
    case 'room'
        missionCounter = 2;
        currentWaypoint = size(navWaypoints,2)/2+1;
        currentRegion = 1;
    case 'return'
        missionCounter = 3;
        currentWaypoint = size(navWaypoints,2)/2+1;
...
for k = 1:iterations
    ...
    for ii = 1:numAgents
        ...
        switch lower(currentSubmission)
            ...
            case 'refuel'
                ...
                missionInfo{ii}.target = finalFormation;
        end
    ...
    vfield = getVelocityField(robotPositions,domBoundary,u);
    switch currentSubmission
        case {'init','return'}
            % vfield = vfield - vmax*xt/max(norm(xt),delta);
        case 'room'
            % roomCenterLine = mean(beacons(1,:));
            % vfield(1) = vfield(1) - vmax*(roomCenterLine)/max(abs(roomCenterLine),vmax);
            % vfield = 1.75*vfield;
        case 'refuel'
            vfield = vfield - 0.25*vmax*(xt)/max(vecnorm(xt),vmax);
    end
    % Offload scenary transition from agent control
    u = u + vfield;
    
    % Scale velocities to actuator bound
    maxSpeed = max(vecnorm(u));
    if maxSpeed > vmax
        u = vmax*u/maxSpeed;
        vfield = vmax*vfield/maxSpeed;
    end
    ...
    switch currentSubmission
        case {'init','return'}
            if norm(robotPositions(:,end)-xt)<delta
                if currentWaypoint < size(navWaypoints,2)/2
...
% imPosWaypoints = [...
%     3.40340  -4.95518  -4.95518  -4.95518  -3.66466   4.2077   4.97000   4.97000;
...
numVerticalTiles = 5;%15;%30;%27;
numHorizontalTiles = 7;%round(diff(regionXLim,1,1)./diff(regionYLim,1,1)*numVerticalTiles);
